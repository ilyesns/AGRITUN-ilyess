...
void triggerPushNotification({
  ...
  int seconds = 0;
  Timer timer = Timer.periodic(Duration(milliseconds: 750), (timer) {
    seconds++;
    if (seconds == 2) {
      Future<List<YdRecord>> ydRecord = queryYdRecordOnce(
          queryBuilder: (ydRecord) =>
              ydRecord.where('sn_device', isEqualTo: snDevice),
          limit: 1);
      ydRecord.then((value) {
        bool val =
            value.first.powers![power] == notificationText ? true : false;
        if (val == true) {
